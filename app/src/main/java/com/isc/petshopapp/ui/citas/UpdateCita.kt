...
class UpdateCita : Fragment() {
  ...
  override fun onCreateView(
    ...
    _binding = FragmentUpdateCitaBinding.inflate(
      inflater,
      container, false
    )
    ...
    /*citaViewModel.currCita.observe(viewLifecycleOwner,{
        cita -> setData(cita)})*/
  ...
  private fun insertaCita() {
    ...
    if (validos(id, descripcion, titulo, fecha, inicio, fin)) {
      ...
      val cita = Cita(id, descripcion, titulo, fecha, inicio, fin)
      citaViewModel.updateCita(cita, context)
      Toast.makeText(
        requireContext(), getString(R.string.citaadd),
        Toast.LENGTH_LONG
      ).show()
      ...
      Toast.makeText(
        requireContext(), getString(R.string.citafail),
        Toast.LENGTH_LONG
      ).show()
    }
  }
